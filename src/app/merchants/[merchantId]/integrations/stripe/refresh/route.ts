...
export async function GET(request: NextRequest, { params }: { params: Promise<{ merchantId: string }> }) {
  ...
  try {
    ...
    if (!session) return NextResponse.redirect(orbitLoginUrl(stripeRefreshPath(merchantId), appOrigin), { status: 303 });
    ...
    try {
      ...
      if (status === 401) return NextResponse.redirect(orbitLoginUrl(stripeRefreshPath(merchantId), appOrigin), { status: 303 });
