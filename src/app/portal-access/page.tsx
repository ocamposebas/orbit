...
export default async function PortalAccessPage() {
  ...
  return <main className="grid min-h-dvh place-items-center bg-[#f4f5f7] px-5 py-12 text-[#1d2025]"><section className="w-full max-w-lg rounded-[24px] border border-[#dfe1e5] bg-white p-7 text-center shadow-[0_24px_70px_rgba(24,28,36,.08)] sm:p-10"><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-[#efecff] text-[#6f60dc]"><Clock3 className="size-5" /></span><p className="mt-6 text-[10px] font-semibold uppercase tracking-[.14em] text-[#8f83df]">ORBIT Merchant Portal</p><h1 className="mt-3 text-[26px] font-semibold tracking-[-.045em]">Your portal access is pending</h1><p className="mx-auto mt-4 max-w-md text-[12px] leading-6 text-[#747982]">Your ORBIT administrator will activate the financial portal after the merchant application, Stripe verification, payments capability and payouts capability are complete.</p>{merchants.length > 0 && <div className="mt-7 rounded-xl border border-[#e6e7ea] bg-[#fafbfc] p-4 text-left"><p className="flex items-center gap-2 text-[10px] font-medium text-[#555a62]"><ShieldCheck className="size-3.5 text-[#7668dc]" />Assigned account{merchants.length === 1 ? "" : "s"}</p><p className="mt-2 text-[10px] leading-5 text-[#888c94]">{merchants.map((merchant) => merchant.businessName).join(", ")}</p></div>}<div className="mt-7"><PortalAccessSignOut /></div></section></main>;
}
