...
export default async function PortalAccessPage() {
  ...
  const merchants = await getDatabase().merchant.findMany({ where: portalMerchantScope(session), select: { id: true, businessName: true, portalEnabled: true, agreement: { select: { status: true } }, stripeConnect: { select: { displayStatus: true, cardPaymentsStatus: true, payoutsStatus: true } } } });
  if (merchants.some((merchant) => (["OWNER", "ADMIN", "ANALYST"].includes(session.role) || (merchant.portalEnabled && portalActivationEligibility({ agreementStatus: merchant.agreement?.status, stripeDisplayStatus: merchant.stripeConnect?.displayStatus, cardPaymentsStatus: merchant.stripeConnect?.cardPaymentsStatus, payoutsStatus: merchant.stripeConnect?.payoutsStatus }).eligible))) || session.role === "OWNER" || session.portalAllMerchants) redirect("/dashboard");
