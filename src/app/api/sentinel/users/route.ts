  ...
  try {
    ...
    const [memberships, merchants] = await Promise.all([
      ...
      db.merchant.findMany({
        ...
        select: { id: true, businessName: true, sites: { where: { active: true }, take: 1, select: { hostname: true } } },
      }),
    ]);
    return NextResponse.json({
      ...
      merchants: merchants.map((merchant) => ({ id: merchant.id, businessName: merchant.businessName, hostname: merchant.sites[0]?.hostname ?? null })),
    });
