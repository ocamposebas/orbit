...
import { StatementPreference } from "@/components/merchant-portal/statement-preference";
...
export default async function MerchantSettingsPage() {
  ...
  return <div className="mx-auto w-full max-w-[1120px] px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
    ...
    {merchant && <section className="mt-4 rounded-[20px] border border-white/[.075] bg-[#0c0e14] p-5"><p className="mb-5 text-[9px] font-semibold uppercase tracking-[.14em] text-[#9182f0]">Financial notifications</p><StatementPreference initialValue={merchant.monthlyStatementEmailEnabled} disabled={!merchant.canManageStatements} /></section>}
    <div className="-mx-4 sm:-mx-7 lg:-mx-10"><ProfileSecurity email={session.user.email} role={session.role} twoFactorEnabled={Boolean(session.user.twoFactorEnabledAt)} /></div>
  </div>;
}
