...
function policyCoverage(scan: AiScanReportData) {
  ...
  const definitions = [
    ...
    { label: "Terms", pattern: /(?:^|\/)(?:terms|terms-of-service|terms-and-conditions|conditions)(?:\/|$)/i },
...
export function aiScanReportHtml(scan: AiScanReportData, imageData = new Map<string, string>(), generatedAt = new Date()) {
  ...
  const policyRows = policies.map((policy) => `<tr><td>${escapeHtml(policy.label)}</td><td><span class="policy-status ${policy.url ? "observed" : "unobserved"}">${policy.url ? "OBSERVED" : "NOT OBSERVED"}</span></td><td class="canonical-url">${policy.url ? escapeHtml(policy.url) : "No matching public URL was present in retained AI Scanner evidence."}</td></tr>`).join("");
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    .severity-pill { padding: 2mm 3mm; border: 1px solid currentColor; font-size: 7px; } .policy-status { font-size: 7px; letter-spacing: .8px; } .policy-status.observed { color: #247356; } .policy-status.unobserved { color: #8b6e36; }
    ...
    .evidence-group { margin-top: 6mm; } .evidence-group h4 { padding-bottom: 2mm; border-bottom: 2px solid #8588e5; color: #62677a; } .evidence-group.adverse h4 { color: #a74646; border-color: #c16b6b; } .evidence-group.mitigating h4 { color: #247356; border-color: #57a183; } .evidence-group.neutral h4 { color: #59647e; border-color: #8790aa; }
    ...
    <section class="section"><div class="section-head"><h2>Key risk themes</h2><span class="section-no">03</span></div><table><thead><tr><th>THEME</th><th>MAX SEVERITY</th><th>FINDINGS</th><th>EVIDENCE</th><th>MITIGATING</th><th>AFFECTED URLS</th></tr></thead><tbody>${themeRows}</tbody></table></section>
    ...
    <section class="section"><div class="section-head"><h2>Policy coverage</h2><span class="section-no">05</span></div><table><thead><tr><th>PUBLIC POLICY SURFACE</th><th>OBSERVED STATE</th><th>RETAINED FIRST-PARTY URL</th></tr></thead><tbody>${policyRows}</tbody></table><p class="lead" style="margin-top:5mm">“Observed” means that a matching public URL appeared in retained, validated AI Scanner evidence. It does not assert that the policy is complete, legally sufficient, applicable, or unchanged.</p></section>
