...
export const evidenceClassificationSchema = z.enum(["ADVERSE", "NEUTRAL", "MITIGATING", "CONTRADICTORY", "INFORMATIONAL"]);
...
function classification(candidate: CandidateFinding) {
  return candidate.evidenceClassification ?? (/contradict|inconsisten/i.test(`${candidate.category} ${candidate.title}`) ? "CONTRADICTORY" : "ADVERSE");
}

function evidenceRecord(candidate: CandidateFinding, input: { url: string; text: string; evidenceType?: string; classification?: "ADVERSE" | "NEUTRAL" | "MITIGATING" | "CONTRADICTORY" | "INFORMATIONAL"; role?: string; sourceKind?: "TEXT" | "VISUAL" | "DOCUMENT" | "INTERACTIVE"; assetStorageKey?: string; assetHash?: string; domSelector?: string }) {
...
export function buildEvidenceGraph(candidates: CandidateFinding[], pageRestrictions: Array<{ url: string; pageType: SentinelPageType; text: string }> = [], generatedAt = new Date()) {
  ...
  const themes = [...groups.entries()].map(([riskTheme, findings]) => {
    ...
    const evidence = findings.flatMap((finding) => [evidenceRecord(finding, { url: finding.url, text: finding.detectedText ?? finding.title, evidenceType: finding.evidenceType, role: "primary" }), ...(finding.supportingEvidence ?? []).map((item) => evidenceRecord(finding, { ...item, classification: item.classification ?? classification(finding) }))]);
