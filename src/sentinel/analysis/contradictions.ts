...
const claimBearingPageTypes = new Set<SentinelPageType>(["HOME", "PRODUCT", "COLLECTION", "CATEGORY", "LANDING", "BLOG", "ARTICLE"]);
...
export function evaluateContradictions(pages: PageInput[]): CandidateFinding[] {
  ...
  if (positioningPage) {
    ...
    for (const page of usablePages.filter((item) => claimBearingPageTypes.has(item.pageType))) {
  ...
  if (exclusionPage) for (const page of usablePages.filter((item) => item.pageType === "PRODUCT" && item.url !== exclusionPage.url && !cosmeticExclusion.test(`${item.content.productName} ${item.content.headings.join(" ")}`) && cosmeticPositioning.test(`${item.content.productName} ${item.content.headings.join(" ")}`))) findings.push({ ruleKey: "POSITION-COSMETIC-001", severity: "HIGH", confidence: 0.91, status: "NEEDS_REVIEW", category: "Positioning conflict", title: "Product positioning conflicts with site policy", description: "A product appears positioned for cosmetic use while another public page excludes that use.", url: page.url, pageType: page.pageType, detectedText: page.content.productName, reason: `The catalog identity conflicts with the explicit exclusion observed on ${new URL(exclusionPage.url).pathname}.`, recommendedAction: "Review product naming, description and site-level policy as one consistent position.", scoreComponent: "OPERATIONAL_CONSISTENCY" });
  for (const page of usablePages.filter((item) => item.pageType === "PRODUCT")) {
    const readMeasurements = (text: string) => [...text.matchAll(/\b\d+(?:\.\d+)?\s*(mcg|mg|g|units?)\b/gi)].map((match) => ({ value: match[0].toLowerCase().replace(/\s/g, ""), unit: match[1].toLowerCase() }));
    ...
    if (headingSet.size === 1 && bodySet.size === 1 && !hasSharedValue && !allDeclaredAsVariants) findings.push({ ruleKey: "PRODUCT-CONCENTRATION-001", severity: "MEDIUM", confidence: 0.84, status: "NEEDS_REVIEW", category: "Product consistency", title: "Potential concentration mismatch", description: "The single concentration shown in the product identity differs from the single same-unit concentration observed in the descriptive copy.", url: page.url, pageType: page.pageType, detectedText: distinct.join(" / "), reason: "One unambiguous heading value and one unambiguous body value were observed, they do not match, and the values were not declared as variants.", recommendedAction: "Verify the product heading, selected variants, structured data and descriptive copy against the source product record.", scoreComponent: "PRODUCT_INTEGRITY" });
  }
  return findings;
}
