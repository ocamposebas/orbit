...
export function calculateHealthScore(findings: CandidateFinding[], assessmentCoverage: Partial<Record<ScoreComponentKey, number>> = {}) {
  ...
  return { total, formulaVersion: "orbit-health-v4", components, explanation: { basis: "Internal ORBIT score derived from material, deduplicated website signals and the evidence coverage achieved by each assessment area.", scale: { minimum: 0, maximum: 100, higherIsBetter: true }, weights, assessmentCoverage, uncertaintyFloor: 70, note: "Uninspected scope cannot create a violation, but it prevents an unsupported perfect score. Repeated instances of the same rule on one page are deducted once. This score is decision support, not a certification." } };
}
