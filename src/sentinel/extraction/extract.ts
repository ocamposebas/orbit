...
const claimTerms = /\b(weight loss|fat loss|burn(?:s|ing)? fat|metabolism|treat|cure|heal|diagnos|dose|dosage|inject|injection|consume|consumption|swallow|sublingual|oral use|topical use|apply topically|serving size|take (?:one|two|three|\d+)|(?:once|twice) (?:daily|weekly)|daily use|for human use|personal use|appetite|anti-aging|performance|body transformation)\b/i;
