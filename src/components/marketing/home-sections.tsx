...
export function SystemSection() {
  return (
    <section id="how-it-works" className="border-b border-white/[.09] bg-[#050611] py-24 sm:py-32">
      <div className="container-shell">
        <Reveal>
          <div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
            ...
            />
            <p className="max-w-xl text-base leading-8 text-[#999bb3] lg:justify-self-end">
          ...
          {system.map(({ number, title, text, icon: Icon }, index) => (
            <Reveal key={number} delay={index * 0.06}>
              <article className="group grid gap-5 border-b border-white/[.1] py-7 sm:grid-cols-[72px_1fr_1.2fr_32px] sm:items-center sm:gap-7">
                <span className="font-mono text-[10px] text-[#a78bfa]">{number}</span>
                <div className="flex items-center gap-3">
                  <Icon className="size-4 text-[#a6a29b]" />
                  <h3 className="text-base font-medium text-[#e9e9f4]">{title}</h3>
                </div>
                <p className="max-w-xl text-sm leading-6 text-[#85879f]">{text}</p>
                <ArrowRight className="hidden size-4 text-[#55576f] transition-transform group-hover:translate-x-1 group-hover:text-[#a78bfa] sm:block" />
              </article>
            </Reveal>
          ))}
        </div>
...
export function MonitoringSection() {
  return (
    <section id="monitoring" className="relative overflow-hidden border-b border-white/[.09] bg-[#080a18] py-24 sm:py-32">
        ...
        <div className="grid gap-12 lg:grid-cols-[.68fr_1.32fr] lg:items-start">
          <Reveal>
            <div className="lg:sticky lg:top-28">
              ...
              />
              <div className="mt-9 border-l border-[#7c5cff]/70 pl-5">
                <p className="font-mono text-[10px] uppercase text-[#797b94]">What ORBIT watches</p>
...
export function FindingsSection() {
  return (
    <section id="findings" className="border-b border-white/[.09] bg-[#050611] py-24 sm:py-32">
      <div className="container-shell">
        <Reveal>
          <div className="grid gap-10 lg:grid-cols-[.75fr_1.25fr] lg:items-end">
            <SectionHeading eyebrow="Focused review" title="Not more alerts. Better decisions." />
            <p className="max-w-2xl text-base leading-8 text-[#999bb3] lg:justify-self-end">
          ...
          {findings.map((finding, index) => (
            <Reveal key={finding.path} delay={index * 0.06}>
              <article className="grid gap-5 border-b border-white/[.1] py-7 md:grid-cols-[90px_1fr_1.15fr] md:gap-8">
                <div>
                  <span className={`inline-flex items-center gap-2 font-mono text-[9px] uppercase ${index === 0 ? "text-[#b8a6ff]" : index === 1 ? "text-[#f0c36b]" : "text-[#5aa7ff]"}`}>
                    <span className="size-1.5 rounded-full bg-current" /> {finding.status}
                  </span>
                </div>
                <div>
                  <p className="text-[10px] font-medium uppercase text-[#797b94]">{finding.type}</p>
                  <p className="mt-2 font-mono text-[9px] text-[#60627a]">{finding.path}</p>
                  <p className="mt-4 text-base text-[#d9dae8]">{finding.quote}</p>
                </div>
                <div className="md:border-l md:border-white/[.08] md:pl-8">
                  <p className="text-[10px] uppercase text-[#6a6c84]">Recommended review</p>
        ...
        <p className="mt-5 max-w-2xl text-[10px] leading-5 text-[#60627a]">Illustrative findings. ORBIT supports qualified human review and does not make legal determinations.</p>
      </div>
    </section>
  );
}
...
export function EvidenceSection() {
  return (
    <section id="changes" className="border-b border-white/[.09] bg-[#080a18] py-24 sm:py-32">
      <div className="container-shell">
        <Reveal>
          <div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-end">
            <SectionHeading eyebrow="Change intelligence" title="The evidence stays attached." />
            <p className="max-w-xl text-base leading-8 text-[#999bb3] lg:justify-self-end">Compare saved states, isolate the new language, and see the reason a change entered the review queue.</p>
          </div>
        </Reveal>
...
export function MerchantHealthSection() {
  return (
    <section className="border-b border-white/[.09] bg-[#050611] py-24 sm:py-32">
      <div className="container-shell">
...
export function BuiltForSection() {
  return (
    <section className="border-b border-white/[.09] bg-[#080a18] py-24 sm:py-32">
      <div className="container-shell">
            ...
            <Reveal key={label} delay={index * 0.06}>
              <article className="group flex min-h-[280px] h-full flex-col border-b border-r border-white/[.1] p-6 transition-colors hover:bg-white/[.02] sm:p-8">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[9px] text-[#585a72]">0{index + 1}</span>
                </div>
                <div className="mt-auto pt-16">
                  <p className="text-[10px] font-medium uppercase text-[#797b94]">{label}</p>
                  <h3 className="mt-3 text-xl font-medium leading-7 text-[#e8e8f3]">{title}</h3>
                  <p className="mt-3 max-w-xs text-sm leading-6 text-[#85879f]">{text}</p>
                </div>
              </article>
            </Reveal>
          ))}
...
export function CustomComplianceSection() {
  return (
    <section className="border-b border-white/[.09] bg-[#050611]">
      <div className="container-shell grid gap-8 py-16 lg:grid-cols-[1.2fr_.8fr] lg:items-center">
        <Reveal>
          <p className="text-[10px] font-medium uppercase text-[#a78bfa]">Beyond detection</p>
          <h2 className="mt-4 max-w-3xl text-3xl font-medium leading-tight text-[#e9e9f4] sm:text-4xl">When the storefront is the risk, we can rebuild the surface too.</h2>
        </Reveal>
        <Reveal delay={0.08}>
          <p className="max-w-xl text-sm leading-7 text-[#8e90a8]">Custom commerce experiences with clear policies, appropriate controls, and compliance-aware language built into the launch.</p>
...
export function SecuritySection() {
  ...
  return (
    <section className="border-b border-white/[.09] bg-[#080a18] py-16">
      <div className="container-shell grid gap-8 lg:grid-cols-[.65fr_1.35fr] lg:items-center">
        <Reveal>
          <div className="flex items-center gap-3"><ShieldCheck className="size-5 text-[#6ee7c3]" /><p className="text-sm font-medium text-[#d9d9e8]">Built for sensitive operations</p></div>
        </Reveal>
        <Reveal delay={0.08}>
          <div className="grid gap-px bg-white/[.09] sm:grid-cols-2">
            {items.map((item) => <div key={item} className="flex items-center gap-3 bg-[#080a18] px-5 py-4 text-xs text-[#9c9eb5]"><CircleCheck className="size-3.5 text-[#6ee7c3]" />{item}</div>)}
          </div>
        </Reveal>
      </div>
    </section>
...
export function FinalCta() {
  return (
    <section className="final-orbit relative isolate overflow-hidden bg-[#050611] py-24 sm:py-36">
      <div className="container-shell relative z-10">
        <Reveal>
          <div className="max-w-5xl">
            <p className="text-[10px] font-medium uppercase text-[#a78bfa]">Keep risk in view</p>
            <h2 className="mt-6 text-5xl font-medium leading-[.9] text-[#f5f4ff] sm:text-7xl lg:text-8xl">Stay ahead of what changes.</h2>
            <p className="mt-7 max-w-xl text-base leading-7 text-[#999bb3]">Build a continuously monitored, evidence-backed view of merchant compliance.</p>
