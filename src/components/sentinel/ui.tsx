...
export function StatusIcon({ status }: { status: string }) { if (status === "COMPLETED") return <Check className="size-3.5 text-[#72c7a1]" />; if (["FAILED", "AI_SCAN_FAILED", "AI_SCAN_INCOMPLETE"].includes(status)) return <AlertTriangle className="size-3.5 text-[#dc8585]" />; return <CircleDot className="size-3.5 text-[#8588f0]" />; }
