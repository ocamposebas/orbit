...
export function LiveScan({ scanId }: { scanId: string }) {
  ...
  const uploadManualReport = async (file?: File) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "Unable to upload the manual PDF report");
    } finally {
  ...
  return <div className="mx-auto max-w-[1280px] px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
    ...
    ].map(([label, value]) => <div key={label} className="bg-[#0c0e12] p-4"><p className="text-[9px] text-[#5f646c]">{label}</p><p className="mt-2 font-mono text-sm text-[#d2d4cf]">{value}</p></div>)}</div><p className="mt-4 text-[9px] leading-4 text-[#555a62]">Coverage reports actual investigation activity. Reaching a runtime, tool, token, or cost budget never implies complete coverage.</p></section>
