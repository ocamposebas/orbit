...
import { Building2, Check, Link2, LockKeyhole, Pencil, Plus, RefreshCw, UserRound, WalletCards, X } from "lucide-react";
...
type WorkspaceUser = {
  ...
  merchantAccess: Array<{ id: string; businessName: string; canInitiatePayouts: boolean; canCreatePaymentLinks: boolean }>;
};
...
export function UserManagement({ currentRole }: { currentRole: string }) {
  ...
  const [editingPaymentLinkMerchantIds, setEditingPaymentLinkMerchantIds] = useState<string[]>([]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      merchantIds: data.getAll("merchantIds"), payoutMerchantIds: data.getAll("payoutMerchantIds"), paymentLinkMerchantIds: data.getAll("paymentLinkMerchantIds"),
    };
    try {
  ...
  function startEditing(user: WorkspaceUser) {
    ...
    setEditingPaymentLinkMerchantIds(user.merchantAccess.filter((merchant) => merchant.canCreatePaymentLinks).map((merchant) => merchant.id));
  ...
  function toggleMerchant(merchantId: string) {
    setEditingMerchantIds((current) => {
      if (current.includes(merchantId)) {
        ...
        setEditingPaymentLinkMerchantIds((links) => links.filter((id) => id !== merchantId));
  ...
  function togglePaymentLinkMerchant(merchantId: string) {
    setEditingPaymentLinkMerchantIds((current) => current.includes(merchantId) ? current.filter((id) => id !== merchantId) : [...current, merchantId]);
  }
  ...
  async function saveAccess() {
    ...
    try {
      await sentinelFetch("/api/sentinel/users", { method: "PATCH", body: JSON.stringify({
        userId: editingUserId, portalAllMerchants: editingAllMerchants, merchantIds: editingMerchantIds, payoutMerchantIds: editingPayoutMerchantIds, paymentLinkMerchantIds: editingPaymentLinkMerchantIds,
      }) });
      setEditingUserId(""); setEditingMerchantIds([]); setEditingPayoutMerchantIds([]); setEditingPaymentLinkMerchantIds([]); setEditingAllMerchants(false); await load();
  ...
  return <div className="mx-auto max-w-[1240px] px-4 py-8 sm:px-7 lg:px-10">
    ...
    <div className="mt-7 grid gap-8 lg:grid-cols-[1fr_390px]">
      <section><div className="flex items-center justify-between"><h2 className="text-sm font-medium">Workspace users</h2><button onClick={() => void load()} aria-label="Refresh users" className="grid size-8 place-items-center rounded border border-white/[.08] text-[#777b84]"><RefreshCw className="size-3.5" /></button></div><div className="mt-3 border border-white/[.075] bg-[#0c0e12]">
        {loading ? <div className="h-40 animate-pulse bg-white/[.025]" /> : users.map((user) => {
          ...
          return <div key={user.id} className="border-b border-white/[.06] last:border-0"><div className="grid gap-3 p-4 sm:grid-cols-[minmax(180px,1fr)_90px_minmax(190px,1fr)_auto] sm:items-center"><div className="flex items-center gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-white/[.045]"><UserRound className="size-3.5 text-[#858991]" /></span><span className="min-w-0"><span className="block truncate text-xs text-[#d0d2cd]">{user.name}</span><span className="mt-0.5 block truncate text-[10px] text-[#62666f]">{user.email}</span></span></div><span className="w-fit rounded border border-white/[.08] px-2 py-1 text-[9px] text-[#8d9199]">{user.role}</span><div><p className="flex items-center gap-1.5 text-[9px] text-[#767a82]"><Building2 className="size-3 shrink-0" />{user.portalAllMerchants ? "All financial brands" : user.merchantAccess.length ? user.merchantAccess.map((merchant) => merchant.businessName).join(", ") : "No financial access"}</p><p className="mt-1 flex items-center gap-1.5 text-[9px] text-[#7770b4]"><WalletCards className="size-3" />{payoutCount ? `Transfers enabled on ${payoutCount} brand${payoutCount === 1 ? "" : "s"}` : "Read-only · no transfers"}</p></div>{canEditFinancial ? <button onClick={() => editing ? setEditingUserId("") : startEditing(user)} className="inline-flex h-8 items-center gap-1.5 rounded border border-white/[.09] px-2.5 text-[9px] text-[#8d9199] hover:text-white">{editing ? <X className="size-3" /> : <Pencil className="size-3" />}{editing ? "Cancel" : "Permissions"}</button> : <span className="text-[8px] text-[#565a63]">Always all</span>}</div>
            {editing && <div className="border-t border-white/[.055] bg-black/10 px-4 py-4"><div className="flex flex-col gap-3 sm:flex-row"><button type="button" onClick={() => setEditingAllMerchants(true)} className={`flex-1 rounded-md border p-3 text-left ${editingAllMerchants ? "border-[#7779ea]/35 bg-[#7779ea]/[.07]" : "border-white/[.07]"}`}><span className="text-[10px] text-[#d0d2cd]">All brands</span><span className="mt-1 block text-[8px] text-[#62666e]">Can see the complete financial portfolio.</span></button><button type="button" onClick={() => setEditingAllMerchants(false)} className={`flex-1 rounded-md border p-3 text-left ${!editingAllMerchants ? "border-[#7779ea]/35 bg-[#7779ea]/[.07]" : "border-white/[.07]"}`}><span className="text-[10px] text-[#d0d2cd]">Selected brands only</span><span className="mt-1 block text-[8px] text-[#62666e]">Can see only checked brands.</span></button></div><div className="mt-4 flex items-center gap-2 text-[9px] font-semibold uppercase text-[#696d76]"><LockKeyhole className="size-3 text-[#8f82ef]" />Brand permissions</div><div className="mt-3 grid gap-2 sm:grid-cols-2">{merchants.map((merchant) => { const visible = editingAllMerchants || editingMerchantIds.includes(merchant.id); const payout = editingPayoutMerchantIds.includes(merchant.id); const paymentLinks = editingPaymentLinkMerchantIds.includes(merchant.id); return <div key={merchant.id} className={`rounded-md border p-3 ${visible ? "border-[#7779ea]/30 bg-[#7779ea]/[.06]" : "border-white/[.07] bg-white/[.015]"}`}><button type="button" onClick={() => !editingAllMerchants && toggleMerchant(merchant.id)} className="flex w-full items-center gap-3 text-left"><span className={`grid size-4 shrink-0 place-items-center rounded border ${visible ? "border-[#8588ef] bg-[#7779ea] text-white" : "border-white/[.14]"}`}>{visible && <Check className="size-2.5" />}</span><span className="min-w-0 flex-1"><span className="block truncate text-[10px] text-[#c2c4bf]">{merchant.businessName}</span><span className={`mt-1 block text-[8px] ${merchant.portalEnabled ? "text-[#6fc39d]" : "text-[#62666e]"}`}>{merchant.portalEnabled ? "Portal activated" : "Portal not activated"}</span></span></button><div className="mt-3 grid gap-2"><button type="button" disabled={!visible} onClick={() => togglePayoutMerchant(merchant.id)} className={`flex w-full items-center justify-between rounded border px-2.5 py-2 text-[8px] disabled:opacity-30 ${payout ? "border-[#8f82ef]/35 bg-[#8f82ef]/10 text-[#c9c5ff]" : "border-white/[.07] text-[#686d76]"}`}><span className="flex items-center gap-1.5"><WalletCards className="size-3" />Can transfer balance</span><span className={`h-3.5 w-6 rounded-full p-0.5 ${payout ? "bg-[#7868e8]" : "bg-white/[.08]"}`}><span className={`block size-2.5 rounded-full bg-white transition ${payout ? "translate-x-2.5" : ""}`} /></span></button><button type="button" disabled={!visible} onClick={() => togglePaymentLinkMerchant(merchant.id)} title="Allows this person to create and publish links without per-link approval." className={`flex w-full items-center justify-between rounded border px-2.5 py-2 text-[8px] disabled:opacity-30 ${paymentLinks ? "border-[#65c9a5]/30 bg-[#65c9a5]/[.07] text-[#8bd3b8]" : "border-white/[.07] text-[#686d76]"}`}><span className="flex items-center gap-1.5"><Link2 className="size-3" />Can create Payment Links</span><span className={`h-3.5 w-6 rounded-full p-0.5 ${paymentLinks ? "bg-[#55b893]" : "bg-white/[.08]"}`}><span className={`block size-2.5 rounded-full bg-white transition ${paymentLinks ? "translate-x-2.5" : ""}`} /></span></button></div></div>; })}</div><div className="mt-4 flex items-center justify-between gap-3"><p className="text-[9px] text-[#555a62]">Changes apply on the next page request.</p><button onClick={() => void saveAccess()} disabled={savingAccess} className="h-8 rounded bg-[#e9eae6] px-3 text-[9px] font-medium text-black disabled:opacity-40">{savingAccess ? "Saving…" : "Save permissions"}</button></div></div>}
          </div>;
        })}
      </div></section>
      <aside><h2 className="text-sm font-medium">Create client access</h2><form onSubmit={submit} className="mt-3 space-y-4 border border-white/[.075] bg-[#0c0e12] p-5">
        ...
        <fieldset><legend className="text-[10px] text-[#7f838b]">Brand permissions</legend><div className="mt-1.5 max-h-64 overflow-y-auto rounded-md border border-white/[.1] bg-black/10 p-2">{merchants.length ? merchants.map((merchant) => <div key={merchant.id} className="rounded px-2 py-2.5 hover:bg-white/[.025]"><label className="flex cursor-pointer items-center gap-3"><input name="merchantIds" value={merchant.id} type="checkbox" defaultChecked={portalAllMerchants} disabled={portalAllMerchants} className="size-3.5 accent-[#7779ea]" /><span className="min-w-0"><span className="block truncate text-[10px] text-[#c2c4bf]">{merchant.businessName}</span><span className={`mt-0.5 block truncate text-[9px] ${merchant.portalEnabled ? "text-[#6fc39d]" : "text-[#555a62]"}`}>{merchant.portalEnabled ? "Portal activated" : merchant.hostname ?? "Portal not activated"}</span></span></label><div className="mt-2 ml-6 flex flex-wrap gap-2"><label className="flex cursor-pointer items-center gap-1.5 rounded border border-white/[.07] px-2 py-1.5 text-[8px] text-[#8f82ef]"><input name="payoutMerchantIds" value={merchant.id} type="checkbox" className="size-3 accent-[#7779ea]" /><WalletCards className="size-3" />Transfers</label><label title="The user may create and publish links without per-link approval." className="flex cursor-pointer items-center gap-1.5 rounded border border-white/[.07] px-2 py-1.5 text-[8px] text-[#71bfa3]"><input name="paymentLinkMerchantIds" value={merchant.id} type="checkbox" className="size-3 accent-[#55b893]" /><Link2 className="size-3" />Payment Links</label></div></div>) : <p className="p-2 text-[10px] leading-4 text-[#5e626a]">Create a merchant before adding financial access.</p>}</div><p className="mt-2 text-[9px] leading-4 text-[#555a62]">Payment Links is disabled by default. Once granted, links can be published without individual approval.</p></fieldset>
