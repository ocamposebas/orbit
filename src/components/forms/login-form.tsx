...
export function LoginForm({ nextPath = "/sentinel" }: { nextPath?: string }) {
  ...
  return <form onSubmit={submit}>
    ...
    <button disabled={busy} type="submit" className="mt-6 flex min-h-11 w-full items-center justify-center gap-2 rounded-[6px] bg-[#7c5cff] px-4 text-sm font-medium text-white transition hover:bg-[#9278ff] disabled:cursor-wait disabled:opacity-60">{busy ? "Signing in…" : "Sign in"}<ArrowRight className="size-3.5"/></button>
