  ...
  return (
    <form onSubmit={submit} className="panel min-w-0 max-w-full rounded-2xl p-5 sm:p-8">
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <button disabled={state === "sending"} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[9px] bg-[#f2f0eb] px-5 text-sm font-medium text-[#090a0c] transition hover:bg-white disabled:cursor-wait disabled:opacity-60" type="submit">{state === "sending" ? "Submitting…" : "Talk to ORBIT"}<ArrowUpRight className="size-3.5"/></button>
        <p aria-live="polite" className={`text-xs ${state === "error" ? "text-[#eca0a0]" : "text-[#78d6ad]"}`}>{state === "success" && <span className="inline-flex items-center gap-2"><CheckCircle2 className="size-3.5"/>Request captured by the local ORBIT endpoint.</span>}{state === "error" && "The local endpoint could not accept the request. Please try again."}</p>
      </div>
      <p className="mt-5 text-[10px] leading-5 text-[#5e6169]">This form currently uses an internal API endpoint. External CRM and email delivery are not connected yet.</p>
    </form>
  );
}
