...
export function RequestAccessForm() {
  ...
  return (
    <form onSubmit={submit} className="panel min-w-0 max-w-full rounded-[6px] p-5 sm:p-8">
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><button disabled={state === "sending"} type="submit" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[6px] bg-[#ff6547] px-5 text-sm font-medium text-[#140806] transition hover:bg-[#ff8068] disabled:cursor-wait disabled:opacity-60">{state === "sending" ? "Submitting…" : "Request access"}<ArrowUpRight className="size-3.5"/></button><p aria-live="polite" className={`text-xs ${state === "error" ? "text-[#eca0a0]" : "text-[#9ee6b3]"}`}>{state === "success" && <span className="inline-flex items-center gap-2"><CheckCircle2 className="size-3.5"/>Your request has been received.</span>}{state === "error" && "We could not accept the request. Please try again."}</p></div>
