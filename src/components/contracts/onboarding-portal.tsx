...
type AgreementState = {
  ...
  intake?: Partial<Record<"businessName" | "publicWebsite" | "industry" | "operatingCountry" | "businessDescription" | "legalName" | "tradeName" | "entityType" | "taxId" | "registrationNumber" | "businessAddress" | "city" | "region" | "postalCode" | "countryCode" | "coveredDomains" | "primaryContactName" | "primaryContactRole" | "primaryContactEmail" | "primaryContactPhone" | "billingDescriptor" | "estimatedMonthlyVolume" | "averageTransactionAmount" | "highestTransactionAmount" | "productsAndServices", string>>;
  completed: boolean;
...
function IntakeForm({ agreement, busy, onSubmit }: { agreement: AgreementState; busy: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const defaults = agreement.intake ?? {};
  return <form onSubmit={onSubmit} className="mt-14 space-y-6">
    <FormSection number="01" title="Business profile" description={agreement.selfServe ? "This information will create your merchant profile and define the website ORBIT will monitor." : "Your existing ORBIT profile is already prefilled. Confirm it and update only anything that is no longer current."}><Field label="Business name" name="businessName" placeholder="Name your customers recognize" defaultValue={defaults.businessName ?? agreement.merchant.businessName} /><Field label="Public website" name="publicWebsite" type="url" placeholder="https://company.com" defaultValue={defaults.publicWebsite ?? agreement.merchant.website} /><Field label="Industry" name="industry" placeholder="Software, e-commerce, professional services..." defaultValue={defaults.industry ?? agreement.merchant.industry} /><Field label="Operating country or market" name="operatingCountry" placeholder="United States, Colombia..." defaultValue={defaults.operatingCountry ?? agreement.merchant.operatingCountry} /><TextField label="Business description" name="businessDescription" placeholder="Describe your products, customers, sales channels and operating model." defaultValue={defaults.businessDescription ?? agreement.merchant.businessDescription} /></FormSection>
    <FormSection number="02" title="Legal identity" description="Only complete or confirm the contractual fields that are not part of your existing monitoring profile."><Field label="Legal business name" name="legalName" placeholder="Full legal entity name" defaultValue={defaults.legalName} /><Field label="Trade name / DBA" name="tradeName" placeholder="Optional" required={false} defaultValue={defaults.tradeName} /><Field label="Entity type" name="entityType" placeholder="LLC, corporation, partnership..." defaultValue={defaults.entityType} /><Field label="Tax identifier" name="taxId" placeholder="EIN, NIT or equivalent" defaultValue={defaults.taxId} /><Field label="Business registration number" name="registrationNumber" placeholder="Optional" required={false} defaultValue={defaults.registrationNumber} /><Field label="Legal country code" name="countryCode" placeholder="US" maxLength={2} defaultValue={defaults.countryCode} /></FormSection>
    <FormSection number="03" title="Address and authorized signer" description="Stripe status does not provide ORBIT with the signer authority and contractual notice details required for this agreement."><Field label="Legal business address" name="businessAddress" placeholder="Street, number and unit" wide defaultValue={defaults.businessAddress} /><Field label="City" name="city" placeholder="City" defaultValue={defaults.city} /><Field label="State / province / region" name="region" placeholder="State or region" defaultValue={defaults.region} /><Field label="Postal code" name="postalCode" placeholder="Postal code" defaultValue={defaults.postalCode} /><Field label="Authorized contact name" name="primaryContactName" placeholder="Full name" defaultValue={defaults.primaryContactName} /><Field label="Role or title" name="primaryContactRole" placeholder="CEO, director, legal representative..." defaultValue={defaults.primaryContactRole} /><Field label="Contract email" name="primaryContactEmail" type="email" placeholder="legal@company.com" defaultValue={defaults.primaryContactEmail} /><Field label="Phone number" name="primaryContactPhone" type="tel" placeholder="+1 555 000 0000" defaultValue={defaults.primaryContactPhone} /></FormSection>
    <FormSection number="04" title="Monitoring scope and operations" description="Existing operating information is reused where available; confirm the values used in the agreement."><TextField label="Covered websites, domains, subdomains and channels" name="coveredDomains" placeholder="Enter one URL or domain per line" defaultValue={defaults.coveredDomains ?? agreement.merchant.website} /><TextField label="Products and services" name="productsAndServices" placeholder="Describe products, audience, sales model, subscriptions, fulfillment and any regulated categories." defaultValue={defaults.productsAndServices} /><Field label="Requested billing descriptor" name="billingDescriptor" placeholder="CUSTOMER STATEMENT NAME" defaultValue={defaults.billingDescriptor} /><Field label="Estimated monthly volume" name="estimatedMonthlyVolume" placeholder="USD 25,000" defaultValue={defaults.estimatedMonthlyVolume} /><Field label="Average transaction amount" name="averageTransactionAmount" placeholder="USD 75" defaultValue={defaults.averageTransactionAmount} /><Field label="Highest expected transaction" name="highestTransactionAmount" placeholder="USD 500" defaultValue={defaults.highestTransactionAmount} /></FormSection>
