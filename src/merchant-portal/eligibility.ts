...
export function portalActivationEligibility(input: PortalEligibilityInput) {
  const requirements = [
    ...
    { key: "connect", label: "Stripe Connect verification enabled", complete: input.stripeDisplayStatus === "ENABLED" },
    { key: "payments", label: "Card payments capability active", complete: input.cardPaymentsStatus?.toLowerCase() === "active" },
    { key: "payouts", label: "Payouts capability active", complete: input.payoutsStatus?.toLowerCase() === "active" },
  ];
