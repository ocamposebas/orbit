...
import { MONTHLY_SERVICE_FEE_USD, PLATFORM_SERVICE_FEE_PERCENT } from "./pricing";

export { MONTHLY_SERVICE_FEE_USD, PLATFORM_SERVICE_FEE_PERCENT } from "./pricing";
...
export function agreementDocumentHtml(agreement: AgreementDocument) {
  ...
  const commercialGrid = [
    field("Monthly service fee", `USD ${MONTHLY_SERVICE_FEE_USD.toFixed(2)}`), field("Platform service fee", `${PLATFORM_SERVICE_FEE_PERCENT.toFixed(1)}% of gross transaction value`),
  ...
  const feeSchedule = `<div class="fee-schedule"><div class="fee-card"><span>Monthly service fee</span><strong><small>USD</small>$${MONTHLY_SERVICE_FEE_USD}</strong><p>Billed monthly in advance for access to ORBIT monitoring, intelligence and agreement services.</p></div><div class="fee-card accent"><span>Platform service fee</span><strong>${PLATFORM_SERVICE_FEE_PERCENT.toFixed(1)}<small>%</small></strong><p>Applied to the gross amount of each successfully processed customer transaction enabled through ORBIT.</p></div></div>`;
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><style>
    ...
    <div class="cover"><div class="cover-top"><div class="brand"><i></i>ORBIT</div><div class="document-chip">Confidential · Master services agreement</div></div><div class="hero"><div class="eyebrow">Contract record · ${escapeHtml(agreement.termsVersion)}</div><h1>Participation,<br>Monitoring &amp;<br>Services Agreement</h1><p>A definitive B2B framework for continuous risk intelligence, commercial monitoring and verifiable operational evidence.</p></div><div class="cover-pricing"><div class="cover-price"><span>Monthly service fee</span><b><small>USD</small>$${MONTHLY_SERVICE_FEE_USD}</b></div><div class="cover-price"><span>Platform service fee</span><b>${PLATFORM_SERVICE_FEE_PERCENT.toFixed(1)}%</b></div></div><div class="cover-foot"><div><span>Merchant</span><b>${escapeHtml(agreement.legalName)}</b></div><div><span>Reference</span><b>${reference}</b></div><div><span>Issued</span><b>${issued}</b></div></div></div>
    <div class="intro"><div class="intro-top"><div><div class="kicker">Parties and certified information</div><h1>${escapeHtml(agreement.legalName)}</h1></div><div class="reference">${reference}<br>${escapeHtml(agreement.termsVersion)}</div></div><p class="lead">This record combines the information certified by the Merchant with ORBIT's uniform commercial terms. The Merchant's authorized representative must review the final document before signing.</p><div class="grid">${partyGrid}</div><div class="subhead"><h2>Commercial profile</h2><span class="terms-badge">USD $${MONTHLY_SERVICE_FEE_USD}/MONTH + ${PLATFORM_SERVICE_FEE_PERCENT.toFixed(1)}%</span></div><div class="grid">${commercialGrid}</div><div class="cert"><b>Declared products and services</b>${escapeHtml(agreement.productsAndServices)}</div><div class="cert"><b>Submitter certification</b>The submitter represented that they have authority to bind the Merchant, certified the accuracy and completeness of this information, and consented to the electronic document process on ${certified}.</div></div>
    ${sections}
    <section class="signatures"><div class="section-head"><span>13</span><h2>Acceptance and signatures</h2></div><p class="signature-intro">By signing, each signatory represents that they have sufficient authority to bind the party they represent, has reviewed this Agreement—including the <b>USD $${MONTHLY_SERVICE_FEE_USD.toFixed(2)} monthly fee</b> and <b>${PLATFORM_SERVICE_FEE_PERCENT.toFixed(1)}% platform service fee</b>—and accepts its terms. Counterpart and electronic signatures have the same effect to the extent permitted by applicable law.</p><div class="signature-grid"><div class="signature-card"><div class="role">For the Merchant</div><h3>${escapeHtml(agreement.primaryContactName)}</h3><div class="title">${escapeHtml(agreement.primaryContactRole)}</div><div class="sign-line">Signature and date</div></div><div class="signature-card provider"><div class="provider-seal">ORBIT</div><div class="role">For ORBIT</div><h3>${REPRESENTATIVE}</h3><div class="title">Authorized representative</div><div class="signature-mark">${signatureMarkup}</div><div class="sign-line">Authorized signature · Applied ${issued}</div></div></div><div class="notice"><b>Integrity record.</b> Generated by ORBIT under reference ${reference}, terms version ${escapeHtml(agreement.termsVersion)}. ORBIT retains the cryptographic hash of the issued file to verify that the signed upload matches this definitive version. Review by qualified legal counsel is recommended before production use in a specific jurisdiction.</div></section>
  </body></html>`;
}
